...
import { User, Mail, Database, LogOut, ArrowRight, KeyRound, CheckCircle2, Lock, Sparkles, RefreshCw } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  if (isPending) {
    return (
      <div className="glass-panel p-12 text-center text-slate-400 max-w-md mx-auto my-12">
        ...
        <p className="text-sm font-semibold text-slate-300">Fetching User Session from Express Backend...</p>
      </div>
    );
  }
  ...
  if (!session?.user) {
    return (
      <div className="glass-panel border-rose-500/20 p-12 text-center max-w-md mx-auto my-12 shadow-2xl">
        ...
        <Link href="/auth?mode=signin" className="btn btn-primary text-xs py-2 px-6 rounded-xl">
  ...
  return (
    <div className="max-w-4xl mx-auto py-6">
      ...
      <section className="page-header mb-6">
        <div className="header-top">
          <span className="px-3 py-1 rounded-full text-xs font-bold bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 flex items-center gap-1.5">
            <Sparkles size={14} className="text-indigo-400" /> Active Session Verified
          </span>
        </div>
        <h1 className="page-title flex items-center gap-3 text-white mt-2">
          <User className="text-indigo-400" size={32} />
          User Profile & Session Telemetry
        </h1>
        <p className="page-description text-slate-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="glass-panel p-6 text-center flex flex-col items-center justify-between shadow-2xl border-indigo-500/30">
          <div>
          ...
          <button onClick={() => signOut()} className="btn btn-secondary text-xs w-full py-2 rounded-xl">
            <LogOut size={14} /> Sign Out Session
          </button>
        </div>

        {/* Right Session Details Card */}
        <div className="glass-panel p-6 md:col-span-2 space-y-4 shadow-2xl border-indigo-500/30">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
            <div className="glass-panel p-3.5 bg-white/5">
              <span className="text-slate-400 uppercase tracking-wider text-[10px] block mb-1 font-semibold">User Identifier</span>
              <span className="font-mono text-indigo-300 break-all font-semibold">{user.id || 'N/A'}</span>
            </div>

            <div className="glass-panel p-3.5 bg-white/5">
              <span className="text-slate-400 uppercase tracking-wider text-[10px] block mb-1 font-semibold">Database Provider</span>
            ...
            <div className="glass-panel p-3.5 bg-white/5">
              <span className="text-slate-400 uppercase tracking-wider text-[10px] block mb-1 font-semibold">ORM & Auth Engine</span>
              <span className="font-semibold text-slate-200">Better Auth + Drizzle ORM</span>
            </div>

            <div className="glass-panel p-3.5 bg-white/5">
              <span className="text-slate-400 uppercase tracking-wider text-[10px] block mb-1 font-semibold">Backend Server</span>
